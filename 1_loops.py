...
for index in range(len(subjects)-1):
    # (sometimes) you have to use -1 because range starts at 0
    print("Subject " + str(index) + ":" + subjects[index])
...
total = 0 
# if print inside the loop everytime the total will reset to zero instad of the previous number added
for number in numbers:
    # add each number to total
    total += number
    # shorthand for total = total + number
print("total: ", total)
# if print inside the loop it will print multiple times on the list
# first time total = 0
# first time total = 0 + 5
# first time total = 5 + 10
# first time total = 15 + 15
# first time total = 30 + 20
...
for number in new_numbers:
    total += number
print("Total sum from 1 to 260000: ", total)
